...
import {
  ContactFormSection,
  type ContactFormSectionDensity,
} from "@/components/contacts/form/ContactFormSection";
import { IconBadge, type IconBadgeTone } from "@/components/ui/icon-badge";
import { cn } from "@/lib/utils";

type ContactFormDisclosureProps = {
  title: ReactNode;
...
export function ContactFormDisclosure({
  ...
  panelId,
  className,
}: ContactFormDisclosureProps) {
  const generatedId = useId();
  const [uncontrolledOpen, setUncontrolledOpen] = useState(defaultOpen);
  const resolvedHeadingId =
    headingId ?? `contact-form-disclosure-heading-${generatedId}`;
  const resolvedPanelId =
    panelId ?? `contact-form-disclosure-panel-${generatedId}`;
  ...
  return (
    <ContactFormSection
      ...
      data-slot="contact-form-disclosure"
