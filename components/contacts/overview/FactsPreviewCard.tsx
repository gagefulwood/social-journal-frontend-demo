...
export function FactsPreviewCard({ facts, onViewAll }: FactsPreviewCardProps) {
  ...
  return (
    <section
      ...
      className="w-full rounded-xl border border-border/70 bg-background/80 p-4 text-left shadow-sm outline-none transition-all duration-200 hover:-translate-y-0.5 hover:border-border hover:bg-muted/30 hover:shadow-md focus-visible:ring-3 focus-visible:ring-ring/50 active:translate-y-0 motion-reduce:hover:translate-y-0"
      onClick={onViewAll}
      onKeyDown={handleKeyDown}
    >
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="flex size-10 items-center justify-center rounded-xl bg-marker-violet text-marker-violet-foreground shadow-sm">
